...
App.prototype.registerEventHandlers = function() {
  document.onkeydown = (event) => {
    ...
    this.lastKeyPressed = keyboardMap[event.keyCode];
  };
  ...
  this.canvas.onmousedown = (event) => {
    if (this.lastKeyPressed == "B") {
      this.scene.bomb(event, this.keysPressed);
  ...
  this.canvas.onmouseup = (event) => {
    ...
    var gameObjects = this.scene.gameObjects;

    for (let i = 0; i < gameObjects.length; i++) {
      if (gameObjects[i].logicalPosition) {
        gameObjects[i].position.set(gameObjects[i].logicalPosition.x, gameObjects[i].logicalPosition.y);
      }
    };
  };
